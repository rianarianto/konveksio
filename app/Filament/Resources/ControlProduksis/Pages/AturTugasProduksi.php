...
class AturTugasProduksi extends Page
{
                    ...
                    ->schema([
                                ...
                                ->schema([
                                    Placeholder::make('technical_specs')
                                        ...
                                        ->content(function () use ($item): HtmlString {
                                            ...
                                            $html .= '<div style="display:flex; align-items:center; gap:12px; padding:12px 16px; border:1px solid #e5e7eb; border-radius:8px;">';
                                                ...
                                                if (!isset($genders[$g]['models'][$mKey])) {
                                                    $genders[$g]['models'][$mKey] = [
                                                        ...
                                                        'attrs' => [
                                                            ...
                                                            'APLIKASI' => !empty($idtl['sablon_jenis']) ? strtoupper($idtl['sablon_jenis']) : '-',
                                                            'LOKASI' => !empty($idtl['sablon_lokasi']) ? strtoupper($idtl['sablon_lokasi']) : '-',
                                                        ]
                                                    ];
                                                }
                                                ...
                                                foreach ($genders as $gCode => $gData) {
                                                    ...
                                                    foreach ($gData['models'] as $mKey => $mData) {
                                                        ...
                                                        if (!empty($mData['custom'])) {
                                                            ...
                                                            $html .= '<div style="font-size:10px; font-weight:800; color:#6b21a8; text-transform:uppercase; margin-bottom:8px;">UKURAN CUSTOM:</div>';
                                                            $html .= '<div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 4px 16px;">';
                                                            foreach ($mData['custom'] as $c)
                                                                $html .= '<div style="font-size:12px; font-weight:700; color:#581c87; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">• ' . htmlspecialchars($c) . '</div>';
                                                            $html .= '</div>';
                                                            $html .= '</div>';
                                                        }
                        ...
                        Group::make([
                            Section::make('Daftar Tugas Produksi')
                                ...
                                ->schema([
                                    Placeholder::make('stage_styles')
                                        ->hiddenLabel()
                                        ->content(fn() => new HtmlString('
                                            <style>
                                                ' . \App\Models\ProductionStage::all()->map(function ($stage) {
                                            $theme = \App\Models\ProductionStage::getThemeColor($stage->name);
                                            $slug = str_replace([' ', '/'], '-', strtolower($stage->name));
                                            return ".fi-fo-repeater-item:has(.stage-{$slug}) { background-color: {$theme['bg']} !important; border: 1px solid {$theme['border']} !important; border-left: 5px solid {$theme['border']} !important; transition: all 0.2s; }";
                                        })->join("\n") . '
                                                /* Style untuk Tugas Baru (Default) */
                                                .fi-fo-repeater-item:has(.stage-tugas-baru) { 
                                                    background-color: #ffffff !important; 
                                                    border: 1px solid #e5e7eb !important; 
                                                    border-left: 1px solid #e5e7eb !important; /* Tanpa garis tebal di kiri */
                                                }
                                                /* Reset header internal style */
                                                .fi-fo-repeater-item-header { background-color: transparent !important; border-bottom: none !important; }
                                                .fi-fo-repeater-item { margin-bottom: 0px !important; border-radius: 12px !important; overflow: hidden; box-shadow: 0 1px 3px 0 rgb(0 0 0 / 0.1); }
                                                .fi-fo-repeater-items { gap: 10 !important; }
                                                /* Rata Bawah untuk Grid Distribusi Size */
                                                .align-bottom-grid > div > .fi-grid,
                                                .align-bottom-grid .fi-grid { 
                                    ...
                                    Repeater::make('productionTasks')
                                        ...
                                        ->schema([
                                            Group::make([
                                                Placeholder::make('stage_header')
                                                    ->hiddenLabel()
                                                    ->content(fn(Get $get) => new HtmlString('<div style="font-weight:800; font-size:15px; color:' . (\App\Models\ProductionStage::getThemeColor($get('stage_name'))['text']) . '; margin-bottom:12px; text-transform:uppercase; letter-spacing:0.05em;">' . ($get('stage_name') ?? 'Tugas Baru') . '</div>')),
                                                Hidden::make('id'),
                                                Grid::make(2)
                                                    ->schema([
                                                        ...
                                                        Select::make('assigned_to')
                                                            ->label('Karyawan')
                                                            ->options(\App\Models\Worker::all()->pluck('name', 'id'))
                                                            ->searchable()
                                                            ->required(),
                                                    ]),

                                                Fieldset::make('Distribusi Qty Per Ukuran')
                                                    ->extraAttributes(['class' => 'align-bottom-grid'])
                                                    ->schema(function (Get $get, Set $set) use ($item) {
                                                        ...
                                                        foreach ($standardSizes as $sz => $max) {
                                                            $fields[] = TextInput::make($sz)
                                                                ...
                                                                ->placeholder('0')
                                                                ->live(debounce: 300)
                                                                ->afterStateUpdated(function ($state, Set $set, Get $get) use ($max, $recalcQty, $sz) {
                                                                    if ($state > $max)
                                                                        $set($sz, $max);
                                                                    $recalcQty($get, $set);
                                                                });
                                                ...
                                                ->extraAttributes(fn(Get $get) => [
                                                    'style' => ' border: 1px solid ' . (\App\Models\ProductionStage::getThemeColor($get('stage_name'))['border']) . ' !important; border-left: 5px solid ' . (\App\Models\ProductionStage::getThemeColor($get('stage_name'))['border']) . '; padding: 20px; border-radius: 12px; margin-bottom: 20px;'
                                                ])
                                        ]) // Tutup Repeater Schema
                                        ->columnSpanFull()
                                        ->itemLabel(function (array $state): ?HtmlString {
                                            $name = $state['stage_name'] ?? 'Tugas Baru';
                                            $slug = str_replace([' ', '/'], '-', strtolower($name));
                                            return new HtmlString('<span class="stage-' . $slug . '" style="font-weight:bold">' . $name . '</span>');
                                        })
    ...
    public function save(): void
    {
        ...
        if ($order = $item->order) {
            ...
            if ($totalTasks > 0 && $order->status === 'diterima')
